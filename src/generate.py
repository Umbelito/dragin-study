...
class BasicGenerator:
    def __init__(self, model_name_or_path):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, device_map="auto",
                    trust_remote_code = "falcon" in model_name_or_path)#.to(device)
...
class AttnWeightRAG(BasicRAG):
    ...
    def keep_real_words(self, prev_text, curr_tokens, curr_hit):
        ...
        input_ids = self.generator.tokenizer.encode(all_text, return_tensors="pt").to('cuda')
        input_length = input_ids.shape[1]
        ...
        for r in range_:
            ...
            att /= (r[1] - r[0] + 1)
            # merge token for att
            att = torch.tensor([att[rr[0]:rr[1]+1].sum() for rr in range_]).to('cuda')
            attns.append(att)
        ...
        for i in range(len(curr_hit)):
            if curr_hit[i] == 1:
                forward_attns += attns[curr_st + i].to(forward_attns.device)
                hit_cnt += 1
